...
function getFizzBuzz(num) {
  ...
  return 'Fizz'.repeat(+!(num % 3)) + 'Buzz'.repeat(+!(num % 5)) || num;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${isStartIncluded ? '[' : '('}${start}, ${end}${
    isEndIncluded ? ']' : ')'
  }`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (opens.includes(str[i])) {
      stack.push(str[i]);
    } else if (closes.includes(str[i])) {
      const char = stack.pop();
      if (code[str[i]] !== char) return false;
    }
  }
...
function getMatrixProduct(m1, m2) {
  // throw new Error('Not implemented');
  const out = [...Array(m1.length)].map(() => [...Array(m2[0].length)]);
...
function evaluateTicTacToePosition(position) {
  ...
  if (!position[1][1]) return undefined;
